...
class DatabaseHelper {
  ...
  void _createDb(Database db, int newVersion) async {
    await db.execute(
        'CREATE TABLE $taskTable ($colId INTEGER PRIMARY KEY AUTOINCREMENT, $colTask TEXT, $colStartDate TEXT, $colEndDate TEXT, $colTimeLeft TEXT ,$colStatus TEXT)');
  }
  ...
  Future<List<Map<String, dynamic>>> getTaskMapList() async {
    ...
    var result = db.query(taskTable, orderBy: '$colStatus, $colStartDate, $colEndDate, $colTimeLeft');
    return result;
  }

  Future<List<Map<String, dynamic>>> getInCompleteTaskMapList() async {
    Database db = await this.database;
    var result = db.rawQuery('SELECT * FROM $taskTable where $colStatus = "" order by $colStartDate, $colEndDate, $colTimeLeft ASC');
  ...
  Future<List<Map<String, dynamic>>> getCompleteTaskMapList() async {
    Database db = await this.database;
    var result = db.rawQuery('SELECT * FROM $taskTable where $colStatus = "Task Completed" order by $colStartDate, $colEndDate, $colTimeLeft ASC');
    //var result = db.query(taskTable, orderBy: '$colStatus, $colDate, $colTime');
    return result;
  }
